...
public class SalaDAO {
    ...
    public Sala doSave(Sala sala) throws SQLException {
        String sql = "INSERT INTO Sala (nome, numeroDiFile, capienza, numeroPostiPerFila) VALUES (?, ?, ?, ?)";
        try (PreparedStatement ps = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            try (ResultSet rs = ps.getGeneratedKeys()) {
                if (rs.next()) sala.setIdSala(rs.getInt(1));
            }
        }
        return sala;
    }
    ...
    private Sala popolaOggetto(ResultSet rs) throws SQLException {
        ...
        s.setConfigurazione(rs.getInt("numeroDiFile"), rs.getInt("numeroPostiPerFila"));
        return s;
    }
}
